...
const { registrar, cargando, error, limpiarError, erroresValidacion } = usarAutenticacion()
const email = ref('')
const clave = ref('')

async function manejarEnvio() {
  await registrar(email.value, clave.value)
}
</script>
...
<template>
  <div class="min-h-screen flex items-center justify-center p-4 bg-fondo-principal">
    <div class="w-full max-w-md bg-fondo-superficie border border-borde rounded-xl p-6 lg:p-8">
      <h1 class="font-secundaria text-2xl font-semibold mb-1">Crear cuenta</h1>
      <p class="text-texto-secundario text-sm mb-6">Registrate para empezar a analizar tus decisiones</p>
      ...
      <form class="flex flex-col gap-4" @submit.prevent="manejarEnvio">
        ...
        <div class="flex flex-col gap-1">
          <label for="clave" class="text-sm font-medium text-texto-primario">Clave (mínimo 8 caracteres)</label>
          <input
            id="clave"
            v-model="clave"
            type="password"
      ...
      <p class="text-center mt-4 text-sm text-texto-secundario">
        ¿Ya tenés cuenta? <router-link to="/iniciar-sesion" class="text-acento hover:text-acento-hover">Iniciá sesión</router-link>
      </p>
    </div>
  </div>
</template>